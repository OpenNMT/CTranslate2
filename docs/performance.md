...
* Packed GEMM is enabled by default on Intel CPUs using the MKL backend, which improves decoding performance. Set `CT2_PACKED_GEMM=0` to disable it if needed.
