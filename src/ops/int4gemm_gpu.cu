...
#pragma unroll
    ...
    void launch_tinygemm_kernel(
        const StorageView& A,
        const StorageView& B,
        const StorageView* qScaleAndZeros, /* optional */
        StorageView& C_final,
        int32_t mTiles,
        int32_t nTiles,
        int32_t kTiles,
      ...
      func<<<grid, block, 0, stream>>>(
          A.data<bfloat16_t >(),
          B.data<int32_t >(),
          qScaleAndZeros ? qScaleAndZeros->data<bfloat16_t>() : nullptr,
          C_final.data<bfloat16_t>(),
          m,
          n,
          k,
          mTiles,
...
#define DECLARE_IMPL(T)                                                 \
    template void                                                       \
    Gemm::compute<Device::CUDA, int32_t, T>(const StorageView& a,       \
                       const StorageView& b,                            \
                       const StorageView& scaleAndZero,                 \
                       StorageView& c) const;
