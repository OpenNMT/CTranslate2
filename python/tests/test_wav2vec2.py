import os, os.path, torch, torchaudio, librosa
...
if not os.path.isfile('ctranslate2_model/model.bin'):
  ...
  converter = ctranslate2.converters.TransformersConverter(model_name,load_as_float16=compute_type)
...
if sampling_rate != resample_rate:
    speech_array = librosa.resample(waveform[0].numpy(), orig_sr=sampling_rate, target_sr=resample_rate)
else:
    speech_array = waveform[0].numpy()
input_values = w2v2_processor(speech_array.astype(np.float32), padding=True, return_tensors="pt", sampling_rate=resample_rate).input_values

with torch.no_grad():
    extract_features = w2v2_model.wav2vec2.feature_extractor(input_values.to(w2v2_model.device)).transpose(1, 2)
    hidden_states, extract_features = w2v2_model.wav2vec2.feature_projection(extract_features)
    ...
    #hidden_states = w2v2_model.encoder.dropout(hidden_states) # Dropout(p=0.0, inplace=False) bypassed
...
hidden_states = hidden_states.cpu() if ct2_w2v2_model.device == "cuda" else hidden_states.numpy()
...
ct2_output = ct2_w2v2_model.encode(hidden_states, to_cpu=to_cpu)
# 24 x Wav2Vec2EncoderLayerStableLayerNorm processed
if ct2_w2v2_model.device == "cuda":
    hidden_states = torch.as_tensor(ct2_output, device=ct2_w2v2_model.device)
else:
    hidden_states = torch.as_tensor(np.array(ct2_output), dtype=torch.float32, device=ct2_w2v2_model.device)
...
#hidden_states = w2v2_model.dropout(hidden_states) # Dropout(p=0.0, inplace=False) bypassed
with torch.no_grad():
...
print(output["text"]) # should be: I HAD THAT CURIOSITY BESIDE ME AT THIS MOMENT
