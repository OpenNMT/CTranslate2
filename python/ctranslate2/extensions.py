...
def register_extensions():
    ...
    setattr(Generator, "async_iterate_batch", generator_async_iterate_batch)
    setattr(Generator, "iterate_batch", generator_iterate_batch)
...
async def generator_async_iterate_batch(
    generator: Generator,
    start_tokens: List[List[str]],
    max_batch_size: int = 32,
    batch_type: str = "examples",
...
def _generate_tokens(process_func, *args, **kwargs):
    ...
    while True:
        if step_results.empty() and generation_done.is_set():
            break
        try:
