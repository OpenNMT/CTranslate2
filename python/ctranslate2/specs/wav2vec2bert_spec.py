...
class ConformerSpec(model_spec.LayerSpec):
    def __init__(self):
...
class Wav2Vec2BertEncoderSpec(model_spec.LayerSpec):
    def __init__(self, num_hidden_layers, num_adapter_layers):
        ...
        self.encoder_layers = [ConformerSpec() for _ in range(num_hidden_layers)]
        self.adapt_layers = [AdapterSpec() for _ in range(num_adapter_layers)]
